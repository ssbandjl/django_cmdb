...
def collect():
    ...
    for key in filter_keys:
        try:
            res = subprocess.Popen("sudo dmidecode -t system|grep '%s'" % key, #res=	Manufacturer: Alibaba Cloud
    ...
    data.update(get_os_info()) #http://www.runoob.com/python/att-dictionary-update.html Python 字典(Dictionary) update() 函数把字典dict2的键/值对更新到dict里。
